...
int main()
{
    ...
    audioEngine.setMusic("res/fortnite.wav");
    audioEngine.startMusicFromBeginning();

    // On charge les effets sonores
    audioEngine.loadSound("res/fortnite.wav");
    audioEngine.loadSound("res/drill.wav");
    audioEngine.loadSound("res/pacman.wav");
    ...
    while (window.isOpen())
    {
        ...
        window << nsGui::Text(nsGraphics::Vec2D(16, 50), "Appuyez sur a pour jouer un  2 eme effet sonore.", nsGraphics::KPurple);
        ...
        if (window.isPressed({'s', false}))
        {
            ...
            const uint8_t soundToPlay = 0;
            audioEngine.playSoundFromBuffer("res/drill.wav");
        }
        if (window.isPressed({'a', false}))
        {
            // A a été pressé, on joue un effet sonore
            window.resetKey({'a', false});
            const uint8_t soundToPlay = 0;
            audioEngine.playSoundFromBuffer("res/pacman.wav");
        }
